...
let studentAnswers = {}; // { taskIndex: { x, y } }
...
function initTeacherAuth() {
    ...
    // Перевірка сесії (якщо вже увійшов у цій вкладці)
...
function initTeacherPanel() {
    ...
    // Опублікувати тест та згенерувати посилання
    if (generateBtn) {
        generateBtn.addEventListener('click', () => {
            ...
            // Збереження у хмару / localStorage
...
// Оновлення текстових інструкцій для вчителя
...
function redrawStudentCanvas() {
    ...
    // Малювання всіх відповідей учня
    Object.keys(studentAnswers).forEach(idx => {
        const pt = studentAnswers[idx];
        const px = (pt.x / 100) * w;
        const py = (pt.y / 100) * h;
        ...
        // Номер завдання біля мітки
        ctx.fillStyle = '#1e293b';
        ctx.font = 'bold 12px sans-serif';
        ctx.fillText(`№${parseInt(idx) + 1}`, px + 10, py + 4);
    });
}

// ==========================================
// 7. ПЕРЕВІРКА ВІДПОВІДЕЙ ТА АЛГОРИТМИ
// ==========================================
    ...
    loadedTest.tasks.forEach((task, index) => {
        ...
        if (ans) {
            ...
            if (task.type === 'marker') {
                ...
                isCorrect = distance <= 7; // Радіус допуску 7%
            } else if (task.type === 'donut') {
                isCorrect = checkDonutAnswer(ans, task.donut);
            }
...
// Перевірка відповіді учня для "Пончика"
function checkDonutAnswer(studentPoint, donut) {
    const inOuter = isPointInPolygon(studentPoint, donut.outer);
    const inInner = isPointInPolygon(studentPoint, donut.inner);

    // Точка має бути всередині ЗОВНІШНЬОГО і поза ВНУТРІШНІМ контуром
